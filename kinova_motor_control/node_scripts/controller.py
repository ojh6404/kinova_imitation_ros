...
class KinovaGen3Controller:
    def __init__(self):
        try:
            ...
            # Get node params
            self.robot_name = rospy.get_param('~robot_name', "arm_gen3")
            # self.robot_name = rospy.get_param('/arm_gen3/arm_gen3_driver/robot_name', "arm_gen3")
            self.degrees_of_freedom = rospy.get_param("/" + self.robot_name + "/degrees_of_freedom", 7) # for gen3
            ...
            self._clear_faults = rospy.ServiceProxy(clear_faults_full_name, Base_ClearFaults)

            read_action_full_name = '/' + self.robot_name + '/base/read_action'
            rospy.wait_for_service(read_action_full_name)
            self._read_action = rospy.ServiceProxy(read_action_full_name, ReadAction)

            execute_action_full_name = '/' + self.robot_name + '/base/execute_action'
            rospy.wait_for_service(execute_action_full_name)
            self._execute_action = rospy.ServiceProxy(execute_action_full_name, ExecuteAction)
            ...
            self._send_gripper_command = rospy.ServiceProxy(send_gripper_command_full_name, SendGripperCommand)
            ...
            rospy.logerr("Initializing Failed")
        else:
            self.is_init_success = True
            rospy.loginfo("Initializing Success")
    ...
    def send_joint_angles(self, angle_vector=None):
        self.last_action_notif_type = None
        ...
        if angle_vector is None:
            angle_vector = [0.] * self.degrees_of_freedom

        angularWaypoint.angles = angle_vector

        print('debugging waypoint')
        # angularWaypoint.angles[1] = 45
        print(angularWaypoint.angles[1])
    ...
    def send_gripper_command(self, value):
        ...
        # Call the service
        try:
            self._send_gripper_command(req)
        except rospy.ServiceException:
    ...
    def cartesian_waypoint_action(self):
        self.last_action_notif_type = None
    ...
    def main(self):
        ...
        if success:
            ...
            # angle vector in deg
            # angle_vector = [0.0] * self.degrees_of_freedom
            # angle_vector[1] = 45.0
            # success &= self.send_joint_angles(angle_vector=angle_vector)
            #*******************************************************************************
            ...
            #*******************************************************************************
            # Let's move the arm
            # success &= self.cartesian_waypoint_action()
            success &= self.send_joint_angles()


        if not success:
            rospy.logerr("Movement failed with error.")


if __name__ == "__main__":
    rospy.init_node('arm_gen3_controller')
    controller = KinovaGen3Controller()
    controller.main()
